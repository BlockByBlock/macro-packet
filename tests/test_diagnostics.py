...
from macro_packet.diagnostics import DRIVER_SHOCK_FLOOR, contradictions, drivers
from macro_packet.engine import indicator_readings
...
def _weekly(series, start, weeks, value_fn):
    return [
        Observation(
            series,
...
def _seed_growth(store, claims_last=None, payems_last=None):
    ...
    store.ingest(_weekly("ICSA", datetime.date(2025, 8, 1), WEEKS,
                         lambda i: claims_last or 220_000 if i == WEEKS - 1 else 220_000))
    store.ingest([
        Observation("PAYEMS", f"2026-{m:02d}-01", 150_000 if m < 8 else (payems_last or 150_000),
                    f"2026-{m:02d}-06")
        for m in range(1, 9)
    ])
    store.ingest(_weekly("UNRATE", datetime.date(2025, 8, 1), WEEKS, lambda i: 4.2))


def _flat(store, series, value):
    store.ingest(_weekly(series, datetime.date(2025, 8, 1), WEEKS, lambda i, v=value: v))
...
def test_driver_list_capped_at_five_and_ranked_by_documented_formula():
    ...
    for series, base in bases.items():
        s.ingest(_weekly(series, datetime.date(2025, 8, 1), WEEKS,
                         lambda i, b=base, l=lasts[series]: l if i == WEEKS - 1 else b))
    result = drivers(s, AS_OF)
    ...
    expected_order = sorted(
        zs, key=lambda n: (-(abs(zs[n] - 0.0)) * specs[n].weight, n)
    )
...
def test_insignificant_days_yield_empty_driver_lists():
    ...
    assert drivers(s, AS_OF) == []
    assert contradictions(s, AS_OF) == []
...
def test_contradiction_flags_cross_signal_disagreement():
    ...
    result = contradictions(store, AS_OF)
    names = [c.series for c in result]
...
def test_contradictions_capped_at_three_strongest_only():
    ...
    def _shock(series, base, last):
        s.ingest(_weekly(series, start, WEEKS, lambda i, b=base, l=last: l if i == WEEKS - 1 else b))

    # R positive: 2y and 10y yields spike while the real yield eases.
    _shock("DGS2", 3.5, 4.8)
    _shock("DGS10", 4.0, 5.3)
    _shock("DFII10", 2.0, 1.0)
    # S positive: VIX jumps while NFCI eases.
    _shock("VIXCLS", 15.0, 40.0)
    _shock("NFCI", 0.0, -0.4)
    # I positive: breakevens rise while oil plunges.
    _shock("T5YIE", 2.3, 2.9)
    _shock("DCOILWTICO", 70.0, 55.0)
